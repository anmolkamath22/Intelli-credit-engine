...
def extract_financials_from_text(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "finance_cost": _num_from_text(r"(?:finance costs?|interest expense|interest cost)[^\d]{0,25}(\d[\d,]*\.?\d*)", text),
        ...
        "current_assets": _num_from_text(r"(?:current assets)[^\d]{0,25}(\d[\d,]*\.?\d*)", text),
        "current_liabilities": _num_from_text(r"(?:current liabilities)[^\d]{0,25}(\d[\d,]*\.?\d*)", text),
